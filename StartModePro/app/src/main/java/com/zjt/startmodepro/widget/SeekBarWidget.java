...
public class SeekBarWidget extends View {
    ...
    public SeekBarWidget(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        Log.e("SeekBarWidget", "attrs = " + attrs);
        if (null != attrs) {
            ...
            lineHeight = typedArray.getDimension(R.styleable.SeekBarWidget_seek_lineHeight, 8f);
...
//        int currColor = colorTransition.getValue(percentage);
        int progressColor = Color.parseColor("#559be9");
    ...
    /**
     * 是否触摸到圆形拖拽按钮，只有触摸到才可以拖拽
     *
     * @param event
     * @return
     */
    private boolean isTouchCircle(MotionEvent event) {
        float x = event.getX();
        final int width = (int) (getMeasuredWidth() - horizontalPadding);
        int circleX = (int) (mPaddingLeft + width * percentage); // 圆形按钮的中心位置
