...
class BaseTransform(
    ...
    fun startTransform() {
        try {
            ...
            print("transform coast time: $timeUsage ms")
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
...
//                                    println("---zjt directory ---- ${classFile.name}， destPath = >>> ${dest.absoluteFile}， absolutePath = $absolutePath")
    ...
    private fun foreachJar(dest: File, jarInput: JarInput) {
        val task = Callable<Void> {
            try {
//                println(">> zjt thread jar >>> ${jarInput.file.name}")
                if (!simpleScan) {
